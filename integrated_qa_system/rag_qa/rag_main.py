...
model_dir_path = os.path.dirname(current_file)
project_root = os.path.dirname(model_dir_path)
sys.path.insert(0, model_dir_path)
sys.path.insert(0, project_root)

from base import Config, logger
from core import process_documents, VectorStore, RAGSystem
from openai import OpenAI

conf = Config()

def main(query_mode=True, directory_path=os.path.join(model_dir_path, 'data')):
    ...
    if not query_mode:
        ...
        for source_dir in conf.VALID_SOURCES:
            ...
            if os.path.exists(dir_path):
                ...
                try:
                    chunks = process_documents(directory_path=dir_path)
                    if chunks:
